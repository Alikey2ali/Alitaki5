...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == '+' then
    return 'همه ی قفل ها فعال بود.'
  else
    data[tostring(target)]['settings']['all'] = '+'
    save_data(_config.moderation.data, data)
    return 'همه ی قفل ها فعال شد.'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  local group_all_lock = data[tostring(target)]['settings']['all']
  if group_all_lock == '-' then
    return 'همه ی تنظیمات باز بود.'
  else
    data[tostring(target)]['settings']['all'] = '-'
    save_data(_config.moderation.data, data)
    return 'همه ی تنظیمات باز شد.'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  if group_all_lock == 'no' then
    return 'همه ی تنظیمات باز بودند.'
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
    return 'همه ی تنظیمات باز شدند.'
  end
end
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "demoteadmin" then
		...
		if result.from.username then
			text = "@"..result.from.username.." has been demoted from admi"
		else
			text = "[ "..user_id.." ] تنزل یافت"
		end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'اضافه' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه از قبل اضافه شده.', ok_cb, false)
			end
		...
		if matches[1] == 'ریمو' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه از روی سرور حذف شد.', ok_cb, false)
			end
		...
		if matches[1] == 'پ' and is_momod(msg) then
		...
		if matches[1] == 'لینک جدید' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, 'ربات باید سازنده گروه باشه تا بتونه لینک رو عوض کنه.')
		...
		if matches[1] == 'تنظیم لینک' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return 'حالا لینک رو بفرس تا ذخیرش کنم.'
		end
		...
		if matches[1] == 'لینک' then
			...
			if not group_link then
				return "یا لینک رو سیو کن یا لینک جدید بساز"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested group link ["..group_link.."]")
			return "بیا اینم لینک:\n"..group_link
		end
		...
		if matches[1] == 'ترفیع' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "فقط سودو و صاحب گروه میتواند ترفیع دهد"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == "تنظیم مشخصات" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == 'پ' then
			...
			if matches[2] == 'لیست مدیران' then
				...
				return 'همه مدیرا پاک شدند.'
			end
			if matches[2] == 'قوانین' then
				local data_cat = 'rules'
				if data[tostring(msg.to.id)][data_cat] == nil then
					return "قوانینی تنظیم نشده عزیزم."
				end
			...
			if matches[2] == 'مشخصات' then
				local receiver = get_receiver(msg)
				local about_text = ' '
				local data_cat = 'description'
				if data[tostring(msg.to.id)][data_cat] == nil then
					return ' مشخصات تنظیم نشده عزیزم.'
				end
				data[tostring(msg.to.id)][data_cat] = nil
				save_data(_config.moderation.data, data)
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] cleaned about")
				channel_set_about(receiver, about_text, ok_cb, false)
				return "مشخصات پاک شدند."
			end
		...
		unlock_group_operator(msg, data, target),
      	...
      	return lock_group_etehad(msg, data, target), etehad
			...
			if matches[2] == 'رباتها' then
				savelog(
msg.to.id, name_log.." ["..msg.from.id.."] locked bots")
		...
		if matches[1] == 'حساسیت' then
			...
			return 'حساسیت رو تغیر دادی به: '..matches[2]
		end
		...
		if matches[1] == 'غیرساکت' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'اهنگ' then
			local msg_type = 'Audio'
		...
		if matches[1] == "ساکت" or matches[1] == "غیرساکت" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
		...
		if matches[1] == "لیست ساکتها" and is_momod(msg) then
			local chat_id = msg.to.id
